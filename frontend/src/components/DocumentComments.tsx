...
const CommentItem = ({
  ...
  return (
    <div
      className={`mb-4 ${isReply ? 'ml-8' : ''} transition-all duration-200`}>
      <div
            ...
            <button
              ...
              <svg
                className={`w-4 h-4 mr-1 transition-transform ${
                  showReplies ? 'transform rotate-90' : ''
                }`}
                fill='none'
                viewBox='0 0 24 24'
                stroke='currentColor'>
...
const DocumentComments = ({
  ...
  const checkPermission = async () => {
    try {
      const response = await api.get(
        `/api/sharing/document/${documentId}/check-permission?action=comment`,
      );
      if (response.data.canAccess) {
        setUserPermission('comment');
        setCanComment(true);
      } else {
        const viewResponse = await api.get(
          `/api/sharing/document/${documentId}/check-permission?action=view`,
        );
  ...
  const fetchComments = async () => {
    ...
    try {
      const response = await api.get(
        `/api/comments/document/${documentId}?includeReplies=true`,
      );
  ...
  const fetchUnreadCount = async () => {
    try {
      const response = await api.get(
        `/api/comments/document/${documentId}/unread-count`,
      );
    ...
    if (loading) {
      return (
        <div className='py-10 text-center'>
          <svg
            className='w-8 h-8 mx-auto text-blue-500 animate-spin'
            xmlns='http://www.w3.org/2000/svg'
            fill='none'
            viewBox='0 0 24 24'>
            <circle
