...
const PDFAnnotationManager = ({ documentId, children }: PDFAnnotationManagerProps) => {
  ...
  const handleAddAnnotation = async (annotationType: 'highlight' | 'underline' | 'note', selectedText: string, position: any) => {
    try {
      const newAnnotation = {
        type: annotationType,
      ...
      const response = await api.post(`/api/annotations/document/${documentId}`, newAnnotation);
  ...
  const renderAnnotationToolbar = () => {
    return (
      <div className="flex items-center p-2 mb-4 space-x-2 bg-white border rounded-md shadow-sm">
        <button
          className={`p-2 rounded-md ${activeAnnotationTool === 'highlight' ? 'bg-blue-100' : 'hover:bg-gray-100'}`}
        ...
        <button
          className={`p-2 rounded-md ${activeAnnotationTool === 'underline' ? 'bg-blue-100' : 'hover:bg-gray-100'}`}
        ...
        <button
          className={`p-2 rounded-md ${activeAnnotationTool === 'note' ? 'bg-blue-100' : 'hover:bg-gray-100'}`}
        ...
        <button
          className={`p-2 rounded-md ${showAnnotations ? 'bg-blue-100' : 'hover:bg-gray-100'}`}
          onClick={() => setShowAnnotations(!showAnnotations)}
          title={showAnnotations ? 'Ocultar anotaciones' : 'Mostrar anotaciones'}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={showAnnotations ? "M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" : "M15 12a3 3 0 11-6 0 3 3 0 016 0z"} />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={!showAnnotations ? "M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" : ""} />
          </svg>
        </button>
      </div>
    );
  ...
  return (
    <div className="pdf-annotation-container">
      ...
      <div 
        ...
        onMouseUp={activeAnnotationTool ? () => handleTextSelection() : undefined}
      >
        {children}
      </div>
      ...
      {showAnnotations && annotations.length > 0 && (
        <div className="mt-4 annotation-list">
          <h3 className="mb-2 text-sm font-medium text-gray-700">Anotaciones ({annotations.length})</h3>
