...
const BiometricRegistration = () => {
    ...
    const checkExistingBiometrics = async () => {
      try {
        ...
        if (response.data.registered) {
          ...
          setError(
            'Ya tienes biometría registrada. Serás redirigido al dashboard.',
          );
  ...
  const handleRegistrationSuccess = async (
    _result: BiometricRegistrationResult,
  ) => {
    if (!user?.id) {
    ...
    try {
      ...
      setError(
        err?.response?.data?.message ||
          'Error al completar el registro biométrico',
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className='px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8'>
      ...
      {step === 'intro' && (
        <div className='p-8 mb-8 bg-white rounded-lg shadow'>
          <h2 className='mb-4 text-xl font-medium text-gray-900'>
            Bienvenido al registro biométrico
          </h2>
          <p className='mb-6 text-gray-700'>
            Para mejorar la seguridad de sus firmas digitales, utilizamos
            verificación biométrica facial. El proceso es simple:
          </p>
          <ul className='mb-6 ml-6 text-gray-700 list-disc'>
            <li className='mb-2'>Posicione su rostro frente a la cámara</li>
            <li className='mb-2'>
              Siga las instrucciones para completar el desafío (parpadeo)
            </li>
            <li className='mb-2'>Confirme y guarde sus datos biométricos</li>
          </ul>
          <div className='p-4 mb-6 rounded-md bg-blue-50'>
            <p className='text-blue-700'>
              <strong>Nota:</strong> Sus datos biométricos se cifran y almacenan
              de forma segura. Puede eliminarlos en cualquier momento desde su
              perfil.
            </p>
          </div>
      ...
      {step === 'complete' && (
        <div className='p-8 text-center bg-white rounded-lg shadow'>
          ...
          <p className='mt-2 text-gray-500'>
            Su biometría ha sido registrada exitosamente. Ahora puede usar la
            verificación biométrica para firmar documentos.
          </p>
          <div className='mt-6'>
      ...
      <div className='p-6 mt-8 rounded-lg bg-blue-50'>
        ...
        <ul className='mt-2 ml-6 text-sm text-blue-700 list-disc'>
          <li className='mt-1'>
            Sus datos biométricos se almacenan de forma cifrada
          </li>
          <li className='mt-1'>
            Puede eliminar sus datos biométricos en cualquier momento
          </li>
          <li className='mt-1'>
            La verificación biométrica cumple con los requisitos de firma
            electrónica avanzada
          </li>
          <li className='mt-1'>
            Sus datos biométricos nunca se comparten con terceros
          </li>
        </ul>
      </div>
    </div>
