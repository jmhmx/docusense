...
const ConfigurationPanel = () => {
  ...
  const [activeTab, setActiveTab] = useState<'email' | 'security' | 'storage' | 'blockchain'>('email');
  ...
  const resetToDefaults = async (section: keyof SystemConfiguration) => {
    if (!window.confirm(`¿Estás seguro de que deseas restablecer la sección ${section} a los valores predeterminados?`)) {
      return;
    }
    ...
    try {
      const response = await api.post(`/api/admin/configuration/reset/${section}`);
      ...
      setSuccessMessage(`Configuración de ${section} restablecida correctamente`);
      ...
      setError(err.response?.data?.message || 'Error al restablecer la configuración');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const testEmailConfiguration = async () => {
    ...
    try {
      ...
      setSuccessMessage('Correo de prueba enviado correctamente. Verifica tu bandeja de entrada.');
      ...
      setError(err.response?.data?.message || 'Error al enviar correo de prueba');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const testBlockchainConnection = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Error al conectar con blockchain');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-4 text-lg font-medium text-gray-700">Cargando configuración...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 bg-white rounded-lg shadow">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Gestiona la configuración de correo electrónico, seguridad, almacenamiento y blockchain.
        </p>
      </div>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="flex items-center mt-6">
                  <input
                    ...
                    onChange={(e) => updateConfig('email', 'useSSL', e.target.checked)}
                    className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                  />
                  <label htmlFor="useSSL" className="block ml-2 text-sm text-gray-900">
                    Usar SSL/TLS
                  </label>
                </div>
                <Input
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <Input
                  ...
                  onChange={(e) => updateConfig('security', 'jwtExpirationHours', parseInt(e.target.value))}
                  fullWidth
                />
                <Input
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) => updateConfig('security', 'requireStrongPasswords', e.target.checked)}
                    className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                  />
                  <label htmlFor="requireStrongPasswords" className="block ml-2 text-sm text-gray-900">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) => updateConfig('security', 'twoFactorAuthEnabled', e.target.checked)}
                    className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                  />
                  <label htmlFor="twoFactorAuthEnabled" className="block ml-2 text-sm text-gray-900">
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    ...
                    <div className="mt-2 text-sm text-yellow-700">
                      <p>
                        Cambiar algunas configuraciones de seguridad puede cerrar las sesiones activas de los usuarios.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <Input
                  ...
                  onChange={(e) => updateConfig('storage', 'maxFileSizeMB', parseInt(e.target.value))}
                  fullWidth
                />
                <Input
                  ...
                  onChange={(e) => updateConfig('storage', 'totalStorageGB', parseInt(e.target.value))}
                  fullWidth
                />
                <Input
                  ...
                  onChange={(e) => updateConfig('storage', 'documentExpirationDays', parseInt(e.target.value))}
                  fullWidth
                />
                ...
                <div className="col-span-2">
                  ...
                  <div className="mt-2">
                      ...
                      {config.storage.allowedFileTypes.map((type, index) => (
                        <div key={index} className="flex items-center px-3 py-1 text-sm bg-gray-100 rounded-full">
                          ...
                          <button 
                            type="button"
                            onClick={() => {
                              const newTypes = [...config.storage.allowedFileTypes];
                      ...
                      <input
                        ...
                        onKeyPress={(e) => {
                          if (e.key === 'Enter') {
                            ...
                            if (value && !config.storage.allowedFileTypes.includes(value)) {
                              updateConfig('storage', 'allowedFileTypes', [...config.storage.allowedFileTypes, value]);
                    ...
                    <p className="mt-2 text-xs text-gray-500">
                      Presiona Enter para añadir un nuevo tipo de archivo. Ejemplos: .pdf, .docx, .jpg
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center mb-6">
                <input
                  ...
                  onChange={(e) => updateConfig('blockchain', 'enabled', e.target.checked)}
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <label htmlFor="blockchainEnabled" className="block ml-2 text-sm font-medium text-gray-900">
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateConfig('blockchain', 'provider', e.target.value)}
                  ...
                  <Input
                    ...
                    onChange={(e) => updateConfig('blockchain', 'apiKey', e.target.value)}
                    placeholder="••••••••"
                    fullWidth
                  />
                  ...
                  <Input
                    ...
                    onChange={(e) => updateConfig('blockchain', 'networkId', e.target.value)}
                    fullWidth
                  />
                </div>
              )}
                  ...
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      ...
                      <div className="mt-2 text-sm text-blue-700">
                        <p>
                          La integración con blockchain permite validar la autenticidad e integridad de los documentos.
                          Los cambios en esta configuración solo afectarán a nuevos documentos.
                        </p>
                      </div>
                    </div>
                  </div>
