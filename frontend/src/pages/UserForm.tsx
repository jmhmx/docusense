...
interface UserFormProps {
  ...
  onSave: (userData: Partial<User>) => Promise<{ error?: string; success?: boolean }>;
  onCancel: () => void;
  isNew: boolean;
}
...
const UserForm = ({ initialData, onSave, onCancel, isNew }: UserFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToSend = !isNew && !formData.password 
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          <Input
            label={isNew ? "Contraseña" : "Contraseña (dejar en blanco para no cambiar)"}
            id="password"
            name="password"
        ...
        {!isNew && (
          <div className="p-4 mt-2 border border-gray-200 rounded-md">
            <h4 className="font-medium text-gray-700">Información de autenticación</h4>
            <div className="flex flex-col mt-2 space-y-2">
              <div className="flex items-center">
                ...
                <span className="text-sm text-gray-600">
                  Autenticación de doble factor: {formData.twoFactorEnabled ? (
                    <span className="font-medium text-green-600">Activado</span>
                  ) : (
                    <span className="font-medium text-gray-500">Desactivado</span>
                  )}
                </span>
              </div>
              ...
              <div className="flex items-center">
                ...
                <span className="text-sm text-gray-600">
                  Autenticación biométrica: {formData.biometricAuthEnabled ? (
                    <span className="font-medium text-green-600">Activado</span>
                  ) : (
                    <span className="font-medium text-gray-500">Desactivado</span>
                  )}
            ...
            <p className="mt-2 text-xs text-gray-500">
              Estos ajustes solo pueden ser modificados por el usuario desde su perfil.
            </p>
          </div>
        )}
      </div>
